...
import { type AiProviderCallConfig, callProvider, type ChatCompletionRequest } from './callProvider.js';
...
async function startMockServer(
  handler: (req: IncomingMessage, body: unknown) => { status: number; body?: unknown; delayMs?: number },
  ...
  if (!address || typeof address === 'string') throw new Error('mock server failed to bind an ephemeral port');
  return { server, baseUrl: `http://127.0.0.1:${address.port}`, receivedAuthHeaders };
}

function baseConfig(baseUrl: string, overrides: Partial<AiProviderCallConfig> = {}): AiProviderCallConfig {
  return {
    baseUrl,
    model: 'test-model',
...
describe('callProvider (T49, AIC-02)', () => {
  ...
  it('produces a structured timeout error_code, never an unhandled throw, when the mock is slow', async () => {
    const { server, baseUrl } = await startMockServer(() => ({
      status: 200,
      body: { id: 'x', model: 'test-model', choices: [{ message: { content: 'late' }, finish_reason: 'stop' }] },
      delayMs: 300,
    }));
    activeServer = server;
  ...
  it('produces a structured http_error error_code (with status) on a non-2xx mock response', async () => {
    const { server, baseUrl } = await startMockServer(() => ({ status: 500, body: { error: 'boom' } }));
    activeServer = server;
  ...
  it('produces a structured malformed_response error_code when the mock returns an unexpected shape', async () => {
    const { server, baseUrl } = await startMockServer(() => ({ status: 200, body: { unexpected: true } }));
    activeServer = server;
  ...
  it('rejects a request whose maxTokens exceeds the configured budget WITHOUT calling the provider', async () => {
    ...
    const { server, baseUrl } = await startMockServer(() => {
      requestCount += 1;
      return { status: 200, body: { id: 'x', model: 'm', choices: [{ message: {}, finish_reason: 'stop' }] } };
    });
    activeServer = server;
  ...
  it('uses the decrypted token only as the outbound Authorization header, never in the returned result', async () => {
    const { server, baseUrl, receivedAuthHeaders } = await startMockServer(() => ({
      status: 200,
      body: { id: 'x', model: 'test-model', choices: [{ message: { content: 'ok' }, finish_reason: 'stop' }] },
    }));
    activeServer = server;
