...
**Status**: ✅ Complete — `apps/server/src/modules/ws-gateway/nodeRestartDurability.int.spec.ts` (1 test, real WS clients, real PGlite). An "old" `FastifyInstance` (its own `InMemoryPresenceBroadcaster`, zero Redis config anywhere in the file) accepts a WS connection and persists two mutations (confirmed by reading `diagram_operations` directly, independent of any in-memory app state); the socket is terminated and the OLD instance is `app.close()`d and its reference dropped entirely — nothing about it (including its `InMemoryPresenceBroadcaster`) survives, simulating a node restart. A brand-new `FastifyInstance` is then built (`bootInstance`, a fresh `registerWsGatewayModule` call with its OWN brand-new `InMemoryPresenceBroadcaster` — never a `RedisPresenceBroadcaster`, never anything resembling `REDIS_URL`) against the SAME PGlite `db` handle the old instance had already written to. A new WS client connects to the new instance, issues `sync_request`, and receives BOTH pre-restart elements with the correct revision — CLB-03, zero durable content lost, reconstructed purely from Postgres. A further mutation on the new instance is accepted and persisted (3rd `diagram_operations` row, the 2 pre-restart client-mutation-ids still present plus exactly 1 new one) — CLB-04, losing presence (trivially true, since presence was never persisted anywhere to begin with) never accidentally turns the restarted node into a read-only server. Follows the same "fresh instance/app, same underlying database, previous instance fully discarded" pattern already established by F1b/F1c's REC/VER integration tests (`catchup.int.spec.ts`/`restore.int.spec.ts`), adapted to two full `FastifyInstance`s instead of one reused instance since the AC specifically requires simulating a NODE restart (all in-memory state gone), not just a fresh read. Full `Gate: build` reproduced from a clean run in this exact order: `pnpm -w lint` (370 files, clean — required 2 Biome auto-format fixes on files this batch touched, `routes.ts`/`crossInstancePresence.int.spec.ts`, plus one unsafe-fix-skipped unused-parameter cleanup done by hand), `pnpm -w typecheck` (22/22 tasks), `pnpm -w build` (12/12 tasks; `grep -rln excalidraw apps/server/dist/**/*.js` → only the same 8 pre-existing allowlisted files from every prior wave, nothing new), `pnpm -w test:unit` (21/21 tasks — server 255/255, web 22/22), `pnpm -w test:integration` (13/13 tasks — server 266/266 across 33 files: +2 from T75's `crossInstancePresence.int.spec.ts`, +1 from T76's `reconnectConvergence.int.spec.ts`, +1 from this task's `nodeRestartDurability.int.spec.ts`, on top of T74's 262/30 baseline). Zero unhandled errors. Confirmed zero lingering `redis-server` processes after the full gate run (`ps aux | grep redis-server` empty) — this task's own test never spawns Redis at all, and T75's spawned process was already torn down by its own `afterAll` earlier in the same run. **Deviation**: none beyond the T75 `routes.ts`/`redisPresence.ts` fixes already documented there (this task's own code is test-only, as its "Where" line specifies).
...
**Status**: ✅ Complete — new module `apps/server/src/modules/share/` (`shareLinks.ts` core CRUD + `isRoleWithinCeiling`/`isShareLinkActive` pure helpers, `routes.ts` registers `registerShareModule`, `index.ts` barrel). `POST /diagrams/:id/share-links` and `POST /presentations/:id/share-links` are two thin endpoints that resolve `resourceType`/`resourceId`/`workspaceId` and both delegate to the same core `createShareLinkForResource` — requires `diagram:mutate` (same bar `publishRoutes.ts` uses for publishing) before even checking the role ceiling, so a reviewer/viewer gets 403 regardless of what `role` they request. `isRoleWithinCeiling(actorRole, grantedRole)` compares positions on `packages/auth`'s `ROLES` (highest-privilege first, index 0) — `grantedRole`'s index must be `>= actorRole`'s index, i.e. never more privileged than the creating actor's own effective role; an `editor` (index 2) can grant `editor`/`reviewer`/`viewer` but a `workspace_admin` request is 403. Token generation/storage reuses `generateOpaqueToken`/`hashToken` (`auth/tokens.ts`, F1a) completely unchanged, exactly as T71's Status note pre-committed to (`secretEncrypted`-vs-`secretHash` decision was about webhooks, T79 — share links always only need one-way hash comparison, matching `sessions`/`ws_tickets`). The plaintext token is returned exactly once in the create response's `token` field; `toPublicShareLink` never serializes `tokenHash` (or anything token-shaped) into ANY route's response, including `:revoke`'s. `GET /share/:token` is the module's only route with no `requireSession` — resolves the token's hash, and folds three distinct failure causes (nonexistent hash, `revokedAt` set, `expiresAt` in the past) into the exact same 404, mirroring `presentation/publish.ts`'s `getPublishedPresentation` IDOR convention exactly. The served resource is capped at the LINK's own `role` — for a diagram, `loadDiagramScene` (F1b) plus `role`; for a presentation, live (not published-snapshot) `presentation`+`frames` with notes redacted via a local `redactNotesUnlessEditor` unless `can({role: link.role}, 'diagram:mutate', ...)` — `link.role` is used for that decision, NEVER a real requester's role (this route is unauthenticated by design, so there is no requester role to fall back to even if it wanted to), which is exactly what keeps a leaked token capped even for a real `workspace_admin` of the same workspace. `POST /share-links/:id(^[^:]+):revoke` (same `:id(^[^:]+)` regex-constrained param convention as `publishRoutes.ts`'s `:publish`) requires either being the link's own creator or holding `workspace:manage_members` (granted only to `workspace_admin`/`org_admin` — reused as the "admin-level" check, same as every other admin gate in this codebase); sets `revokedAt`, idempotently. 9 unit tests (`shareLinks.spec.ts`, pure `isRoleWithinCeiling`/`isShareLinkActive` — every ROLES pair boundary + revoked/expired/inclusive-boundary cases) + 10 integration tests (`share.int.spec.ts`, real PGlite, `app.inject`, no WS needed): role-ceiling rejection AND within-ceiling success; no-`diagram:mutate` rejection; token never recoverable again (checked against the raw persisted row via `JSON.stringify` substring search, not just the typed fields, plus both the create AND revoke response bodies); expired/revoked/nonexistent token all 404 identically; a leaked token still serves only the link's capped role even to a real `workspace_admin` teammate; `:revoke` immediately invalidates; `:revoke` by a non-creator non-admin is 403; presentation share link redacts notes at `viewer` but not at `editor`; IDOR 404 for a diagram outside the actor's workspace. `pnpm --filter @arch-canvas/server exec vitest run src/modules/share/shareLinks.spec.ts` → 9/9; `pnpm --filter @arch-canvas/server exec vitest run -c vitest.integration.config.ts src/modules/share/share.int.spec.ts` → 10/10. `pnpm -w lint` clean (375 files — 1 Biome auto-format/import-sort pass applied to the 4 new files), `pnpm -w typecheck` clean (22/22 tasks), `pnpm -w build` green (12/12; `grep -rln excalidraw apps/server/dist/**/*.js` → same 8 pre-existing allowlisted files, nothing new — `share` module never touches `SceneElement`/element diffs at all, only opaque `scene: unknown[]` passthrough from `loadDiagramScene`), `pnpm -w test:unit` green (21/21 tasks — server 267/267, +9 from `shareLinks.spec.ts`; web 22/22). Gate for this task is `quick` per its own declared field — `test:unit` run as required; lint/typecheck/build/integration above are additional evidence, not a gate substitution. **Deviation**: none — the task's "Where" line lists only `apps/server/src/modules/share/`; `registerModules.ts` wiring (L-008) is explicitly T81's job (a later batch out of this session's scope), same precedent T74's own Status note already established for `ws-gateway`.
...
**Status**: ✅ Complete — new module `apps/server/src/modules/webhook/` (`webhooks.ts` core CRUD + `WEBHOOK_EVENT_TYPES`/`isWebhookEventType`/`listEnabledWebhookEndpointsForEvent` query helpers, `routes.ts` registers `registerWebhookModule`, `index.ts` barrel). `GET/POST/PATCH/DELETE /workspaces/:id/webhooks[/:webhookId]` plus `PATCH /workspaces/:id/webhooks/:webhookId(^[^:]+):rotate-secret` — every route gated by `assertWorkspaceAdmin` (`workspace:manage_members`, held exclusively by `workspace_admin`/`org_admin` among all 5 roles, the exact same admin check `export/routes.ts`'s EXP-04 bulk-export route and `share/routes.ts`'s `:revoke` path already use — located and reused unchanged before writing any new admin-check logic, per the task's own instruction). **Deps shape deviation (necessary)**: the task's shorthand `registerWebhookModule(app, { db })` omits the master key the envelope encryption needs — `WebhookModuleDeps` adds `encryptionKey: string`, mirroring `AiProviderModuleDeps`'s identical requirement in `ai-provider/routes.ts` (F2a) exactly; documented here since it's the first deviation from the task's literal signature. Secret generation reuses `generateOpaqueToken` (auth/tokens.ts) for the random value and `encryptToken`/`decryptToken` (`@arch-canvas/ai-tools`, located via `ai-provider/routes.ts`'s existing import — same module T71's Status note pre-committed to) for storage — never `hashToken`, since the delivery worker (T80) must recover the plaintext secret to compute an outbound HMAC signature. The plaintext secret is returned exactly once, both at creation and at `:rotate-secret`; `toPublicWebhookEndpoint` never serializes `secretEncrypted` (or a `secret` field) into any response, including list/patch. `:rotate-secret` replaces the stored ciphertext outright (no dual-secret grace period, documented simplification that still satisfies "rotacionável"). `eventsJson` (`events` in the request/response body) is validated via `z.array(z.enum(WEBHOOK_EVENT_TYPES)).min(1)` against the exact 5 event types from `docs/product-spec.md` §7.3. `DELETE` removes a webhook's `webhook_deliveries` rows first (no `ON DELETE CASCADE` on that FK per T71's schema) before deleting the endpoint row itself, avoiding an FK-violation footgun. A webhook resolved under the wrong `:id` workspace in the URL is a 404 (IDOR), never a 403 that would confirm it exists elsewhere. 2 unit tests (`webhooks.spec.ts`, pure `WEBHOOK_EVENT_TYPES`/`isWebhookEventType` coverage) + 8 integration tests (`webhook.int.spec.ts`, real PGlite, `app.inject`): non-admin editor rejected on create (403, nothing persisted) AND on patch/rotate/delete of an existing webhook (403, 3 sub-assertions); admin create succeeds, secret is 20+ chars, never appears in the persisted row's `JSON.stringify`, never echoed by create/list responses; `:rotate-secret` produces a different secret from before, never echoes the old one, and an HMAC signature computed with the OLD secret over a fixed payload does not match one computed with the NEW secret (Node's own `crypto.createHmac`, proving "no longer valid for subsequent deliveries" at the crypto-primitive level, independent of T80's not-yet-built delivery pipeline); an out-of-enum event type is 400; `PATCH` updates url/events/enabled while leaving `secretEncrypted` byte-for-byte unchanged; `DELETE` then a subsequent `PATCH` 404s; a webhook from workspace A resolved via workspace B's URL is 404 (IDOR). `pnpm --filter @arch-canvas/server exec vitest run src/modules/webhook/webhooks.spec.ts` → 2/2; `pnpm --filter @arch-canvas/server exec vitest run -c vitest.integration.config.ts src/modules/webhook/webhook.int.spec.ts` → 8/8. `pnpm -w test:unit` green (21/21 tasks — server 272/272, +2 from `webhooks.spec.ts`; web 22/22). `biome check --write` applied one formatting fix to `routes.ts` (a multi-line `.refine()` call); `tsc --noEmit` clean. Gate for this task is `quick` per its own declared field — `test:unit` run as required; the unit+integration numbers above are additional evidence, not a gate substitution (full `lint`/`typecheck`/`build`/`test:integration` for the whole tree is deferred to T80/T81's own declared `build` gates, consistent with every prior task in this wave). **Deviation**: none beyond the documented `encryptionKey` deps addition above.
...
**Status**: ✅ Complete — new `apps/server/src/modules/webhook/deliver.ts`: `signWebhookPayload` (Node's native `crypto.createHmac('sha256', secret).update(serializedPayload).digest('hex')`, `sha256=<hex>` header format, researched directly against Node's `crypto` docs before writing — Knowledge Verification Chain), `enqueueWebhookEvent(db, jobs, workspaceId, eventType, payloadJson)` (T79's `listEnabledWebhookEndpointsForEvent` + one `pending` `webhook_deliveries` insert per matched endpoint + conditional job enqueue), and `deliverWebhookDelivery`/`registerWebhookDeliveryJob` (the `deliver-webhook` pg-boss job, registered with the EXACT optional-`deps.jobs` shape read from `compaction.ts`/`bulkBundle.ts` before writing any of this, per the task's own instruction). **Retry scheduling deviation (documented, not a gap)**: rather than building a second polling worker that scans `(status, next_retry_at)` (which the task's "Where"/T71's schema comment both speculatively anticipated), a failed delivery re-enqueues the SAME pg-boss job using pg-boss's own native `startAfter` option (`SendOptions`, confirmed via `node_modules/pg-boss/dist/types.d.ts` before use) — `jobs/queue.ts`'s `enqueue` helper gained an optional 4th `options` parameter (backward compatible; `enqueueCompaction`/`enqueueBulkWorkspaceBundle`'s existing call sites are untouched) to carry it. This stays fully inside AD-006 ("jobs via pg-boss over Postgres, no bespoke scheduler") and avoids a second, redundant scheduling mechanism; the `(status, next_retry_at)` index remains valid and useful for observability/reconciliation queries even though it isn't the retry timer's own driving mechanism. Backoff: `BACKOFF_SCHEDULE_MS = [1min, 5min, 30min, 2h, 12h]`, `MAX_DELIVERY_ATTEMPTS = 5` — documented mapping in the file: the delay at index `attempts-1` is what's waited BEFORE the next attempt (attempt 1 fails → wait 1min before attempt 2 … attempt 4 fails → wait 2h before attempt 5); attempt 5 failing goes straight to `dead_letter` (the listed 12h entry is intentionally never scheduled, since there's no 6th attempt — documented as deliberate, not an off-by-one bug). A delivery whose endpoint has since vanished dead-letters immediately (defensive; structurally unreachable in practice since `webhook_deliveries.webhook_endpoint_id` has no `ON DELETE CASCADE` and T79's `deleteWebhookEndpoint` always deletes an endpoint's own deliveries first — documented as safety-net code, not exercised in the integration suite for that reason). **Event-wiring granularity decision (documented per the task's own explicit request)**: `diagram.updated` fires exactly ONCE per successful `POST /diagrams/:id/operations:batch` call (`diagram-sync/routes.ts`), never once per individual delta inside that batch — the batch itself is already the client-chosen save unit, so firing per-delta would multiply webhook volume by scene-edit granularity instead of by "the client saved". The metadata-only `PATCH /diagrams/:id` route (title/description/status) deliberately does NOT fire `diagram.updated` — only canvas-content batches do — since `docs/product-spec.md §7.3` describes this event stream as covering the collaboration/webhook use case (canvas changes), and the existing `recordAuditEvent('diagram.updated', ...)` audit-log call already covers metadata changes as a separate, pre-existing concern. `comment.mentioned` fires ONCE per comment that resolves at least one mention (all mentioned userIds bundled into one event's `mentions` array), not once per mentioned user — they were all mentioned by the same comment at the same instant. The 5 wiring sites, each threading an optional `jobs?: JobQueue` dep through its module (same optional-degrade convention everywhere else) and calling `enqueueWebhookEvent` right after its own success path: `diagram.created` (`workspace/project-diagram-routes.ts`'s `POST /diagrams`, `ProjectDiagramModuleDeps` gained `jobs?`, threaded from `WorkspaceModuleDeps`/`registerWorkspaceModule`), `diagram.updated` (`diagram-sync/routes.ts`'s `operations:batch`, reusing its own already-present `jobs` dep), `diagram.published` (`presentation/publishRoutes.ts`'s `:publish`, `PresentationPublishModuleDeps` gained `jobs?`), `spec.generated` (`docgen/routes.ts`'s `specs:generate`, `DocgenModuleDeps` gained `jobs?`), `comment.mentioned` (`comment/routes.ts`'s `POST /diagrams/:id/comments`, `CommentModuleDeps` gained `jobs?`). None of these 5 modules import from `webhook/index.ts` — each imports `enqueueWebhookEvent` directly from `webhook/deliver.js` to avoid pulling `webhook/routes.ts`'s own `workspace/index.js` import back into a cycle (verified no cycle exists: `deliver.ts`'s only local imports are `auth/db.js`, `jobs/index.js`, `./webhooks.js`). 2 unit tests (`deliver.spec.ts`, pure `signWebhookPayload` determinism/correctness against a manual `createHmac` computation) + 5 integration tests (`deliver.int.spec.ts`, real PGlite, mocked `fetch`/`jobs`): a 200 response marks `delivered` with a header signature verifiable via the same secret; a 500 response walks all 5 attempts with strictly-growing `nextRetryAt` gaps then `dead_letter`s with `nextRetryAt` cleared and no 6th attempt; a thrown network error is treated identically to a non-2xx; a disabled endpoint or one not subscribed to the event type gets zero delivery rows; supplying `jobs` re-enqueues the same job with `startAfter` set to a `Date`. 3 more integration tests (`eventWiring.int.spec.ts`, full `app.inject` end-to-end through the real REST routes, never calling `enqueueWebhookEvent` directly): all 5 event sites each insert exactly one `webhook_deliveries` row for an endpoint subscribed to all 5 events (plus a same-diagram, no-mention comment confirmed to add nothing to the `comment.mentioned` count); an endpoint subscribed to a different event type gets nothing for `diagram.created`; a `enabled: false` endpoint gets nothing for any event. `pnpm --filter @arch-canvas/server exec vitest run src/modules/webhook/deliver.spec.ts` → 2/2; `pnpm --filter @arch-canvas/server exec vitest run -c vitest.integration.config.ts src/modules/webhook/deliver.int.spec.ts src/modules/webhook/eventWiring.int.spec.ts` → 8/8. Full `Gate: build` reproduced clean: `pnpm -w lint` (384 files, clean after 2 Biome auto-fixes to the new test files — a multi-arg call reflow and a `noNonNullAssertion` cleanup done by hand), `pnpm -w typecheck` (22/22 tasks), `pnpm -w build` (12/12; `find apps/server/dist -name "*.js" | xargs grep -l excalidraw` → only the same 8 pre-existing allowlisted files from every prior wave, nothing new — the webhook module never touches `SceneElement`), `pnpm -w test:unit` (21/21 tasks — server 275/275, +2 from `deliver.spec.ts`, web 22/22), `pnpm -w test:integration` (13/13 tasks — server 292/292 across 37 files, +8 from `deliver.int.spec.ts`/`eventWiring.int.spec.ts` — `registerModules.int.spec.ts` unaffected/still green since `webhook`/`ws-gateway`/`share` wiring into `registerAllModules` is explicitly T81's job, same precedent T74/T78 already established). Zero lingering `redis-server` processes after the full run. **Deviation**: the `startAfter`-based retry scheduling documented above (chosen over a bespoke poller); no other functional deviation.
...
**Status**: ✅ Complete — `apps/server/src/core/registerModules.ts` now registers `ws-gateway`/`share`/`webhook` (`registerAllModules` also threads `deps.jobs` through to `registerWorkspaceModule`/`registerDocgenModule`/`registerPresentationPublishModule`/`registerCommentModule`, T80's 4 REST-side event-wiring sites that needed it) — chosen `PresenceBroadcaster` is `RedisPresenceBroadcaster(config.redisUrl)` when set, `InMemoryPresenceBroadcaster` otherwise, injectable via a new `deps.presence` for tests. `apps/server/src/core/config.ts` gained `AppConfig.redisUrl` (optional `REDIS_URL` env var, following the exact same optional-field pattern as every other config value here) — 2 new unit tests in `config.spec.ts` (absent by default, carried through when set). `deps.jobs` (when present) also registers `registerWebhookDeliveryJob` alongside the pre-existing `registerCompactionJob`/`registerBulkBundleJob`. `infra/compose/compose.yaml` adds a `redis` service (`redis:7-alpine`, no volume/persistence — CLB-04 already guarantees presence never needs to survive a restart) plus `REDIS_URL: redis://redis:6379` on `server`'s environment, with a comment mirroring the existing `/health/ready`-stub comment's style; `server`'s `depends_on` gets a NON-health-gated `redis: condition: service_started` entry (unlike postgres/minio's `service_healthy`, since redis is explicitly optional/degradable) — validated with `docker compose config` (no daemon needed, confirmed the rendered YAML is well-formed and `REDIS_URL`/the new `redis` service/`depends_on` all resolve as intended). **Real-boot smoke test** (this wave's first WS surface to get one, done rigorously per the task's own instruction): started the sandbox's actual pre-installed Postgres 16 (`service postgresql start`; a genuine `dpkg`-installed cluster, distinct from AD-007's PGlite-for-tests — this is a real `postgres` binary listening on a real socket), created the `arch_canvas` role/database matching `config.ts`'s own documented dev defaults, ran the exact same `packages/database/dist/migrate.js` invocation `infra/compose/migrate.Dockerfile` uses against it. `pnpm -w build`, then `node apps/server/dist/index.js` for real with `DATABASE_URL`/`NODE_ENV`/`PORT`/`SESSION_SECRET`/`ENCRYPTION_KEY` env vars (T70's established pattern, `tasks-f3.md`) — booted clean, logged its real listening addresses. `curl` against the live process, no session cookie: `GET /health/live` → 200 (control), `GET /nonexistent-route-xyz` → 404 (control, proves the 401s below are real auth, not a router miss), `POST /diagrams/x/share-links` → 401, `GET /workspaces/x/webhooks` → 401, `POST /workspaces/x/webhooks` → 401, `GET /share/not-a-real-token` → 404 (correct — this is the module's ONE deliberately-public/unauthenticated route, IDOR-uniform 404 per T78, never a session-gated 401), `GET /ws/diagrams/x?ticket=bogus` (no WS upgrade headers, no real ticket) → 401 (ws-gateway's `preValidation` ticket check rejects before any upgrade, confirmed via `curl -i` showing a plain `application/problem+json` 401 body, not a WS frame). **Real WebSocket connection** (Node's own `ws` package, already a dependency since T73 — a one-off script run from `apps/server/`, deleted immediately after, never committed): seeded a user/workspace/project/diagram via direct calls into the COMPILED `dist/modules/auth/accounts.js`/`dist/modules/workspace/{workspaces,projects,diagrams}.js` against the same real Postgres; `POST /auth/login` (real HTTP, real Argon2id password verify) → real `Set-Cookie` session; `POST /diagrams/:id/ws-ticket` (real HTTP, real session) → a real single-use ticket; opened `ws://127.0.0.1:.../ws/diagrams/:id?ticket=<real ticket>` and received a real `hello` frame within the connection's first message, `payload.userId`/`payload.diagramId` matching the seeded fixtures exactly. Repeated the ENTIRE sequence a second time with a real spawned `redis-server` and `REDIS_URL` set, to prove the `RedisPresenceBroadcaster` code path (not just the default in-memory one) also boots clean and completes the same real handshake — `hello` received identically. Both smoke-test server processes and the ad hoc `redis-server` were killed afterward; `ps aux | grep redis-server` empty. `spec.md`'s Requirement Traceability: EXT-02 flipped `Pending` → `Implementing` (T79/T80 citation); CLB-01..04/EXT-01 left exactly as Batch 2 already set them (`Implementing`, unchanged — this task's own diff never touches their code paths). **Full `Gate: build`** reproduced clean, in order: `pnpm -w lint` (384 files, clean), `pnpm -w typecheck` (22/22 tasks), `pnpm -w build` (12/12; `find apps/server/dist -name "*.js" | xargs grep -l excalidraw` → only the same 8 pre-existing allowlisted files from every prior wave, nothing new), `pnpm -w test:unit` (21/21 tasks — server 276/276 including the 2 new `config.spec.ts` tests, web 22/22), `pnpm -w test:integration` (13/13 tasks — server 294/294 across 37 files, +2 from `registerModules.int.spec.ts`'s new share/webhook 401 checks — every module's own dedicated int-spec suite, including all of T73-T80's, still green unchanged). Zero lingering `redis-server` processes after the full run. **Deviation**: none — `registerModules.ts`'s new `deps.presence` injection point and the `jobs` threading through `WorkspaceModuleDeps`/`DocgenModuleDeps`/`PresentationPublishModuleDeps`/`CommentModuleDeps` (needed for T80's event wiring to actually reach a job queue in production) were both anticipated by those tasks' own Status notes, not scope creep introduced here.
