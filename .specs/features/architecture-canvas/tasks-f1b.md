...
**Post-batch orchestrator fix (2026-08-12, after this batch's gate was already green):** the batch worker's note above correctly identified the `roughjs/bin/rough` resolution failure but scoped its fix to the E2E harness only (`vite-node`). Attempting to wire `apps/server/src/index.ts` — the real production entrypoint, which **no task in F0/F1a/F1b ever touched** — surfaced two compounding, pre-existing gaps: (1) `index.ts` never called any module's register function at all, so a real `docker compose up` server would 404 on every route except `/health/*`; (2) once wired, the same `roughjs` resolution crash broke the real server boot under plain `node`, not just the E2E harness, because `packages/diagram-domain` pulled in `@arch-canvas/editor-adapter`'s `applyRemote` (and therefore `@excalidraw/excalidraw`) at the value-import level. Both fixed: `fix(diagram-domain): decouple server-side reconcile from excalidraw runtime` (`e8b7bf6`) replaces that import with a local, dependency-free LWW reimplementation (design.md's own stated boundary for this package); `fix(server): wire feature modules into the real production entrypoint` (`a8d8927`) adds `DATABASE_URL` to config, a shared `registerAllModules` used by both `index.ts` and a new integration test, and a real postgres `/health/ready` check. Independently verified by booting the compiled `dist/index.js` under plain `node` and curling `/health/live` (200) and `/me` (401, no session) — the real server is reachable end-to-end for the first time since F0. Full gate (lint/typecheck/build/unit/integration/e2e) green after.
