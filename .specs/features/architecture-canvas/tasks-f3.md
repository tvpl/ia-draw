...
**Status**: ✅ Complete — `registerDocgenModule(app, { db, storage })` in `apps/server/src/modules/docgen/routes.ts`, wired into `apps/server/src/core/registerModules.ts` in this same commit (L-008). `POST /diagrams/:id/specs:generate` loads the materialized scene (`materializeScene`, snapshot/F1c) + every `diagram_elements_meta` row (new `listElementMetadata` helper added to `library/metadata.ts`, reused unchanged by T64), calls `extractSceneSemantics` (T58), and renders 4 fixed Markdown sections (Visão Geral/Componentes/Fluxos/Decisões — `docgen/sections.ts`) that cite real `elementId`s. Every field with no canvas/metadata backing renders the literal `"não especificado"`/`"pergunta aberta"` placeholder (DOC-03) — decisions are only ever surfaced from an explicit `semantics.decision` string, never invented. The Markdown is stored in the `exports` bucket (`markdownKey`) and a `spec_documents` row is inserted transactionally alongside flipping every prior `'current'` row for the diagram to `'superseded'` (`insertNewSpecVersion`, reused unchanged by T63). `GET /diagrams/:id/specs` lists versions, newest-first, with `{cursor, limit}` pagination (`nextCursor` — no prior cursor-list convention existed in the codebase to match, so this task establishes one: an opaque `version` cursor). RBAC follows the exact snapshot/export precedent: `diagram:mutate` for `:generate` (403 for reviewer), `diagram:read` for the list (200 for reviewer), IDOR-safe 404 for non-members. `sourceRevision` is stamped from `materializeScene`'s own returned revision (the true "current revision," per the established T39 precedent — `diagrams.current_revision` is a known-unused column in this codebase, not `diagram-sync`'s source of truth), confirmed byte-exact in `docgen.int.spec.ts`. 7 unit tests (`sections.spec.ts`) + 6 integration tests (`docgen.int.spec.ts`, PGlite/AD-007) cover every Done-when bullet plus IDOR. `pnpm --filter @arch-canvas/server exec tsc --noEmit`, full `test:unit` (190/190) and full `test:integration` (204/204, including `registerModules.int.spec.ts` proving the new route is reachable through the real wiring path) all green; `biome check` clean on every touched file. **Deviation**: none functional. **Scope note**: `regenerateSpecSection`/the `:regenerate-section` route (DOC-04) is deliberately NOT part of this commit — it is T63's own file (`regenerateSection.ts`) per the task's own "Where," built on top of `insertNewSpecVersion`/`loadSceneSemantics` exported here for exactly that reuse.
...
**Commit**: `feat(docgen): add single-section spec regeneration with immutable versioning`
**Status**: ✅ Complete — `apps/server/src/modules/docgen/regenerateSection.ts` (`regenerateSpecSection`), wired into the existing `registerDocgenModule` in `routes.ts` (`POST /diagrams/:id/specs/:version(^[^:]+):regenerate-section`, same colon-suffix param-boundary regex pattern already used by `snapshot`'s `:restore` route). Reuses T62's per-section builders directly (`buildOverviewSection`/`buildComponentsSection`/`buildFlowsSection`/`buildDecisionsSection`, re-exported from `sections.ts`) via a local `buildSection` dispatcher — never a duplicated section implementation — and T62's `insertNewSpecVersion`/`loadSceneSemantics` for the write path, so the immutable-version discipline (new row + new storage object, base row/object never touched) is identical code, not parallel code. `parseMarkdownSections` (this file) is the inverse of T62's `assembleMarkdown`, recovering the exact prior text of the 3 unregenerated sections so they come out byte-identical in the new version — proven directly in both `regenerateSection.spec.ts` (pure round-trip, 4 tests) and `regenerateSection.int.spec.ts` (full HTTP flow: regenerate "Fluxos" against a diagram that gained a new resolved edge, assert Componentes/Visão Geral/Decisões are byte-identical and Fluxos changed, assert the base version's storage object is untouched, 5 tests: happy path, unknown-section 400, sourceRevision-advances, reviewer 403, unknown-base-version 404). `sourceRevision` is stamped from `loadSceneSemantics`'s live revision at regeneration time (may have advanced past the base version's), confirmed byte-exact. `pnpm --filter @arch-canvas/server exec tsc --noEmit`, full `test:unit` (195/195) and full `test:integration` (209/209) all green; `biome check` clean. **Deviation**: the task file has no explicit `**Commit**:` line for T63 (only T62 through T70 otherwise do) — authored one following the same Conventional Commit style as every sibling task in this file.
...
**Status**: ✅ Complete — `lintDiagram(scene, semantics, elementsMeta, workspaceRules?)` in `apps/server/src/modules/lint/engine.ts`, a pure function (AD-008, `SceneElement` type-only) implementing 8 rules, each returning at most one aggregated `LintWarning` (`severity` always `'warning'`, never blocks): `orphan-component`, `unclear-direction` (arrows with an unresolved binding, detected from the RAW scene since T58's `extractSceneSemantics` already drops them from `edges`), `connector-no-protocol`, `missing-trust-boundary` (>1 sensitive-`dataClassification` component outside a `frame` whose metadata `semanticType === 'trustBoundary'`), `spof` (>1 incoming edge + no other component sharing its `semanticType`), `secret-in-label` (regex heuristic), `mixed-environments`, and `c4-level-mismatch` (LNT-02: an element whose metadata `c4Level` is more than one abstraction step finer than the diagram's coarsest declared level — soft warning, e.g. Component detail inside a Context diagram). `GET /diagrams/:id/lint` (`routes.ts`, wired into `registerModules.ts`) is `diagram:read`-gated (200 for viewer/reviewer, IDOR-safe 404), always 200 regardless of warning count. `workspaceRules` (LNT-03) is read from `workspaces.settingsJson.lintRules` and a `false` entry suppresses exactly that rule, others unaffected. 11 unit tests (`engine.spec.ts`, covering every rule + a purity assertion) + 4 integration tests (`lint.int.spec.ts`). **Deviation (schema)**: the batch brief's premise that `workspaces.settingsJson` already existed was wrong — only `organizations.settingsJson` existed; `workspaces` had no settings column at all. Added `workspaces.settings_json` (jsonb, `default '{}'`, `NOT NULL`) via a new one-column migration (`infra/migrations/0009_tiny_changeling.sql`, `drizzle-kit generate`), mirroring `organizations.settingsJson` exactly — this is the only schema change in T62-T66, confirmed additive and backward-compatible (`packages/database`'s own integration suite, unchanged, still 29/29 green; `migrate()` applies cleanly from scratch in a fresh PGlite instance in `lint.int.spec.ts`'s own `beforeAll`). **Full `Gate: build` run** (batch instruction: T64 declares `Gate: build` in this file, run in full, not downgraded): `pnpm -w lint` (found and fixed pre-existing Biome formatting drift in drizzle-kit's generated `infra/migrations/meta/*.json` for the new migration, same class of issue T61 already documented), `pnpm -w typecheck` (12 packages, all green — required rebuilding `@arch-canvas/database`'s `dist/` so its published `.d.ts` picked up the new column before `apps/server` could typecheck against it), `pnpm -w build` (12 packages green; `grep -rn "excalidraw" apps/server/dist/modules/{lint,docgen}/*.js` → zero matches), `pnpm -w test:unit` (209/209) and `pnpm -w test:integration` (213/213, including `registerModules.int.spec.ts` and the new `docgen`/`regenerateSection`/`lint` int specs) all green.
...
**Status**: ✅ Complete — `registerPresentationModule(app, { db })` in `apps/server/src/modules/presentation/routes.ts`, wired into `registerModules.ts`. `POST/GET /presentations` (by `diagramId`), `GET/PATCH /presentations/:id` (added beyond the task's literal route list — needed to fetch/edit a single presentation with its frames; documented here as the reasonable completion, same spirit as other modules' CRUD), `POST /presentations/:id/frames` (create), `PATCH /presentations/:id/frames` (bulk reorder, body `{ frames: [{id, position}] }`), `PATCH`/`DELETE /presentations/:id/frames/:frameId` (single-frame edit/delete). RBAC: `diagram:mutate` for every write (403 for reviewer, confirmed in the same test as a successful `GET`), `diagram:read` for reads, IDOR-safe 404 throughout via a shared `resolvePresentationContext` helper that joins presentation → diagram → workspace → role. `navLinksJson` is validated server-side on every write (`assertNavLinksResolve` in `frames.ts`) — a `targetFrameId` not already belonging to the same presentation throws `InvalidNavLinkError` (400), never persisted. `notes` are stripped (`null`) from every response unless the requester holds `diagram:mutate` (T59's schema comment: "privadas — nunca retornadas a um viewer sem permissão de edição"), verified for both a reviewer (redacted) and the owner (not redacted) in the same test file. 6 integration tests (`presentation.int.spec.ts`) covering all 4 Done-when bullets plus notes-redaction and IDOR. `pnpm --filter @arch-canvas/server exec tsc --noEmit` and `test:integration` (this module's spec + `registerModules.int.spec.ts`) green; `biome check` clean. **Deviation**: none beyond the documented `GET /presentations/:id` addition; `setPublishedSnapshot` in `presentations.ts` is exported now but unused by this task's own routes — added for T66 (`publish.ts`) to reuse this table's write path unchanged, per the "Reuses" discipline.
...
**Status**: ✅ Complete — `registerPresentationPublishModule(app, { db, storage })` in `apps/server/src/modules/presentation/publishRoutes.ts`, wired into `registerModules.ts` alongside T65's `registerPresentationModule` (both share the presentation module, split into two `register*` functions purely because this one needs a `StorageClient` T65's CRUD routes never touch). **Signature check (task's own explicit ask)**: read `snapshot/snapshots.ts` before writing any code here — `createSnapshot` already accepted `kind` as a parameter and `IMMUTABLE_KINDS` already included `'published'` since F1c; no signature change was needed, confirmed by reusing it completely unchanged in `publish.ts`'s `publishPresentation` (documented in that file's own top comment so the next reader doesn't re-litigate it). `POST /presentations/:id:publish` creates the immutable snapshot and links `publishedSnapshotId` via a new `setPublishedSnapshot` in `presentations.ts` (T65's file, extended here). `GET /presentations/:id/published` (`diagram:read`-gated, viewer-and-up) serves `getPublishedPresentation`'s view — frames with `notes` always stripped and the scene read straight from the published snapshot's OWN storage object (`storage.getObject` on `snapshot.sceneJsonKey`), never `materializeScene`/`diagram_operations` — proven directly by an integration test that mutates the live scene after publishing and asserts the published response is byte-for-byte unchanged. `settingsJson.expiresAt` (ISO string) in the past collapses to the same `PresentationNotPublishedError` 404 as "never published" — a viewer is never told WHY a link stopped resolving. `POST /presentations/:id:export-pdf` crops the published scene per frame (`elementId` → Excalidraw's own `frameId` element-membership field, `frameId`-only logical frames fall back to the whole scene per page — documented heuristic, no geometric extent to crop by), renders each crop with `render/svg.ts`'s `renderSceneToSvg` (F1c/T10), and concatenates pages via a NEW `svgPagesToPdfBuffer` in `export/pdf.ts` — `svgToPdfBuffer` is now `svgPagesToPdfBuffer([svg])` internally, so the single- and multi-page paths can never drift apart (verified: page-count/MediaBox-per-page assertions in `pdf.spec.ts`, byte-identity intentionally NOT asserted since pdfkit embeds a per-call `/ID`+`/CreationDate`). Uploads to the `exports` bucket, returns a signed URL + `pageCount`. 5 new unit tests (`pdf.spec.ts` additions) + 6 integration tests (`publish.int.spec.ts`, all 4 Done-when bullets plus unpublished-export-404 and reviewer/viewer RBAC) — all green. **Full `Gate: build` run**: `pnpm -w lint`, `pnpm -w typecheck` (12 packages), `pnpm -w build` (`grep -rn "excalidraw" apps/server/dist/modules/presentation/*.js` → zero matches, AD-008 held), `pnpm -w test:unit` (219/219), `pnpm -w test:integration` (225/225, including `registerModules.int.spec.ts`) — all green. **Deviation**: `render/index.ts` gained one more re-export (`RenderElement`, the single-element type — was defined in `svg.ts` but not previously re-exported from the module barrel) so `exportPdf.ts` could type the frame-cropping filter without reaching past the barrel; no behavior change.
...
**Status**: ✅ Complete — `WIREFRAME_LOFI_ITEMS` in `packages/library-content/src/presets/wireframe-lofi.ts` (4 items: `wireframe.screen.blank`, `wireframe.button.primary`, `wireframe.input.text`, `wireframe.list.item`), each a valid `LibraryItem` (original CC0-1.0 inline SVG, flat gray `#6B7280` stroke, straight corners — no hi-fi styling per the task's own instruction), wired into `LIBRARY_MANIFEST` in `manifest.ts` (`rawManifest.items` now spreads `WIREFRAME_LOFI_ITEMS` alongside the existing generic/AWS items). Purely data — no route code touched in `apps/server/src/modules/library/` or `packages/diagram-ir`, confirmed by re-running the existing suites unchanged. 3 new unit tests (`presets/wireframe-lofi.spec.ts`): >=4 items with unique/resolvable `stableKey`s including the 4 required ones, every item schema-valid with `category: 'wireframe'` + license/attribution, and every item's `stableKey` present in the compiled `LIBRARY_MANIFEST`. `manifest.spec.ts` (pre-existing, T37) stays green unmodified in its per-item assertions; **Deviation**: `CATEGORIES` needed a 13th value (`'wireframe'`) since `category` is a closed Zod enum — added it, and split the original 12 into a new exported `SOURCE_DOCUMENT_CATEGORIES` constant so `manifest.spec.ts`'s "covers all 12 source-document categories" test keeps checking exactly those 12 (unaffected by the new category), plus one new assertion for the 13-category count — documented in `schema.ts`'s doc comment. AC #3 ("an existing `library` module integration test can list/find the new preset by category `wireframe` with zero code changes") verified via the pre-existing, unmodified `packages/database/src/library.int.spec.ts` ("`seedGlobalLibrary` inserts every item of the T37 manifest as `library_items`") — it iterates `LIBRARY_MANIFEST.items` generically by `stableKey`, so the 4 new wireframe items are picked up automatically; re-ran green (29/29, `pnpm --filter @arch-canvas/database test:integration`), plus `apps/server/src/modules/library/library.int.spec.ts` (10/10) and `pnpm --filter @arch-canvas/library-content test:unit`/`build`/`typecheck` all green.
...
**Status**: ✅ Complete — `registerInteropModule(app, { db, storage, jobs })` in `apps/server/src/modules/interop/routes.ts`, wired into `registerModules.ts` in this same commit (L-008), right after `registerExportModule` (dependency order: reuses `export/import.ts`'s established write path). `importDsl.ts`'s `importDiagramFromDsl` parses via T60/T61's `{ ir, limitations }` contract, creates a brand-new diagram (`createDiagram`), compiles the IR to a scene via `compile(ir, [])` (F2b — always safe with an empty library since neither DSL parser ever emits a `componentKey`), and seeds the diagram's first revision via the exact `buildRestoreDeltas([], elements)` + `appendOperation` write path `export/import.ts`'s `.excalidraw` `confirmImport` already established. `exportDsl.ts`'s `buildBestEffortIr` builds a best-effort `IrDocument` from `extractSceneSemantics` (T58) plus the same raw `diagram_elements_meta` rows docgen/lint already read — every heuristic field (fixed `kind: 'c4-context'` default, always-empty `containers`, elementId-fallback labels, bound-text-child exclusion from nodes, edge `mode`/`protocol`/`direction` read from the connecting arrow's own metadata defaulting to `dependency`/`oneway`) is documented inline in the file's top comment, then rendered via T60/T61's `toMermaidFlowchart`/`toStructurizrDsl` unchanged — so a lossy edge (e.g. `mode: 'data'`) is flagged in `limitations` by T60/T61's own code, never re-invented here. **Deviation (routing, 2 parts)**: (1) same reasoning `export/routes.ts` already documents for `.excalidraw` import — the task text names import `/diagrams/{id}/import:mermaid`, but import creates a NEW diagram (no diagram id exists yet); registered under `/projects/:id/import:mermaid`/`:structurizr` instead, mirroring the established `/projects/:id/import` precedent exactly. (2) registering `/projects/:id/import:mermaid` and `/projects/:id/import:structurizr` as two separate Fastify routes throws `FST_ERR_DUPLICATED_ROUTE` — find-my-way parses a bare `:name` anywhere in a path segment as a parameter declaration (not literal text), so the two routes collide at the same trie position under different parameter names; confirmed empirically with an isolated Fastify repro before writing the fix. Resolved by registering ONE route per verb with a single real `:format` path parameter (`/projects/:id/import:format`, `/diagrams/:id/export:format`) and branching on its value in the handler (`parseFormatParam`, which strips the leading colon find-my-way's capture includes) — the public URL shape (`.../import:mermaid`, `.../import:structurizr`) is unchanged for callers, documented inline in `routes.ts`. Also added `diagram-ir`'s `src/interop/index.ts` re-export to the package's main `src/index.ts` (T60/T61 only exported it from the interop-local barrel, unreachable from `@arch-canvas/diagram-ir`'s single `"."` package.json export) — same pattern `extractSceneSemantics` already uses from `@arch-canvas/diagram-domain`. 7 unit tests (`exportDsl.spec.ts`, pure `buildBestEffortIr`/`exportDslFromScene`, no DB) + 11 integration tests (`interop.int.spec.ts`, PGlite/AD-007) covering all 5 Done-when bullets plus RBAC (import: viewer 403, export: IDOR 404) and unauthenticated 401. `pnpm --filter @arch-canvas/server exec tsc --noEmit`, this module's `test:unit`/`test:integration`, and the full workspace `pnpm -w build` + `grep -rn "excalidraw" apps/server/dist/modules/interop/*.js` (zero real import/require, only doc-comment mentions) all green; full `pnpm --filter @arch-canvas/server test:integration` re-run afterward (236/236, including `registerModules.int.spec.ts`) confirms no collision with any other module's routes. **Gate note**: task declares `Gate: quick`; ran `test:unit` plus this module's own `test:integration` directly as due diligence (same precedent T65/T62/T63 documented), with the full workspace `Gate: build` deferred to T70 as instructed.
...
**Status**: ✅ Complete — `registerCommentModule(app, { db })` in `apps/server/src/modules/comment/routes.ts`, wired into `registerModules.ts` in this same commit (L-008), after `registerPresentationPublishModule`. `POST/GET /diagrams/:id/comments` (create — a reply is just a create with `parentId` set; list — flat, oldest-first, each row carrying its own `parentId`, thread reconstruction left to the caller, documented on `listComments`) and `PATCH /diagrams/:id/comments/:commentId` (status open/resolved and/or the author's own `body` edit). `packages/auth/src/rbac.ts` gained `comment:create`/`comment:resolve` as a third, independent action axis (never derived from `diagram:write`/`diagram:mutate`) granted to ALL 5 roles per CMT-01's plain "WHEN a user comments" wording (no role restriction) and CMT-02's explicit reviewer requirement; `packages/auth/src/rbac.spec.ts`'s decision matrix grew from 8×5=40 to 10×5=50 rows plus a new dedicated isolated (no-HTTP) test asserting `can({role:'reviewer'}, 'comment:create'/'comment:resolve', ...)` allowed and `can({role:'reviewer'}, 'diagram:mutate', ...)` denied in the same test, exactly as the task requires. Mentions (`mentions.ts`): a simple `@userId`(uuid)/`@email` regex, resolved against the diagram's live workspace membership (`listWorkspaceMembers`, F1a) — an unresolvable mention (non-member, typo) is silently dropped from the returned `mentions` array while `body` is always persisted verbatim, never failing the comment. A `parentId` is validated to reference an existing comment on the SAME diagram (400 otherwise) — a reasonable data-integrity guard in the same spirit as T65's `navLinksJson` validation, beyond the task's literal ACs. Body edits via PATCH are additionally gated by authorship (only the original author may edit their own text), since `comment:resolve`/`comment:create` are uniformly granted to every role and can't by themselves distinguish "edit MY comment" from "edit ANYONE's". 10 unit tests (`mentions.spec.ts`) + 8 integration tests (`comment.int.spec.ts`, PGlite/AD-007) covering all 4 Done-when bullets — including the thread-survives-a-simulated-restart test via a BRAND NEW `drizzle(client, {schema})` connection wrapping the same underlying PGlite storage (re-reads directly, no in-process cache involved) — plus cross-diagram `parentId` rejection, PATCH authorship enforcement, and IDOR/401. **Full `Gate: build` run** (task's own declared gate, not downgraded): `pnpm -w lint`, `pnpm -w typecheck` (22 packages), `pnpm -w build` (`grep -rn "excalidraw" apps/server/dist/modules/comment/*.js` and `packages/auth/dist/*.js` → zero matches, AD-008 held), `pnpm -w test:unit` (244/244) and `pnpm -w test:integration` (244/244 across 27 files, including `registerModules.int.spec.ts`) — all green. **Deviation**: none functional beyond the `parentId` cross-diagram guard noted above.
