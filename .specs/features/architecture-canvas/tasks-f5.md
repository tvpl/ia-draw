...
**Status**: ✅ Complete — researched `prom-client@15.1.3`'s CURRENT installed API directly from `node_modules/.pnpm/prom-client@15.1.3/node_modules/prom-client/index.d.ts` before writing any code (Knowledge Verification Chain): `Registry`/`Counter`/`Gauge`/`Histogram` are plain classes over a `{ name, help, labelNames?, registers? }` config; `Registry#metrics()` returns `Promise<string>` and awaits every metric's own async `collect()` callback, which is exactly the mechanism used for the job-queue-depth gauge (samples pg-boss's `getQueueStats` LIVE on every scrape, never a bespoke poller — mirrors T90's `PgBoss#schedule` research discipline). New `apps/server/src/core/metrics.ts`: a `MetricsRegistry` class owning one `prom-client` `Registry` and 9 documented series — `arch_canvas_http_request_duration_seconds`/`_http_errors_total` (REST, labels `method`/`route`/`status_code`), `arch_canvas_mutation_ack_duration_seconds` (labels `transport`, the SAME histogram for both REST and WS), `arch_canvas_job_queue_depth` (labels `queue`, live `collect()`), `arch_canvas_snapshot_compaction_duration_seconds`, `arch_canvas_ai_run_duration_seconds`/`_ai_run_tokens_total`/`_ai_run_estimated_cost_usd_total`, `arch_canvas_export_duration_seconds` — plus a `declare module 'fastify' { interface FastifyInstance { metrics } }` augmentation (same pattern `auth/types.ts` already establishes for `request.authContext`).

`core/server.ts`: `buildServer` now decorates `app.metrics` (injectable via `BuildServerOptions.metrics`, defaults to a fresh registry), adds an `onResponse` hook observing every REST response's latency + (for 5xx) error count keyed by `request.routeOptions.url` (the route PATTERN, e.g. `/diagrams/:id/bootstrap` — never the raw URL, avoiding unbounded label cardinality from path params), and registers `GET /metrics` (no session requirement, same `/health/*`-style disclosure comment: must be network-restricted in a real deployment). `registerModules.ts` reads `app.metrics` back (never constructs a second registry) and threads it into `registerDiagramSyncModule`/`registerWsGatewayModule` (mutation ACK, `transport: 'rest'|'ws'` respectively — both observe into ONE histogram, proving F4's "WS is just a second transport" invariant at the metrics layer too), `registerExportModule` (export duration, labeled `'bundle'` since `generateExports` produces all 4 formats in one call — documented scope decision, not a per-format breakdown), `registerAiEngineModule`→`pipeline.ts`'s `createAiRun` (duration + token counts + estimated-cost, read straight from the SAME `usageJson` object already persisted to `ai_runs.usage_json`, at all 3 of the pipeline's exit points — provider-call failure, tool-call failure, and success), and wires `metrics.setJobQueueDepthSource(deps.jobs, [COMPACT_DIAGRAM_JOB, BULK_WORKSPACE_BUNDLE_JOB, WEBHOOK_DELIVERY_JOB])` when `deps.jobs` is present (T90's `RESTORE_TEST_JOB` deliberately excluded — its own job registration is T96's job, same precedent T79/T80's webhook module already established, so its queue name is added to this list there too, not here). `snapshot/compaction.ts`'s `compactDiagram` observes its own duration in a `finally` block (captured even on a thrown error, not just the successful path).

**Estimated AI cost disclosure (documented in `metrics.ts`'s own doc comment, not silently approximated as exact)**: `ESTIMATED_COST_PER_1K_TOKENS_USD` is a single flat, disclosed-as-rough rate — AIC-01..03 already establish that this system's AI providers are arbitrary operator-configured/self-hosted OpenAI-compatible endpoints, not one metered vendor with a queryable real-time price, so an exact cost figure is structurally unobtainable here. Mirrors AIC-04/PRS-01's own precedent for "disclosed, not silently approximated" scope notes.

Tests: `core/metrics.spec.ts` (7 unit tests) — valid Prometheus text-exposition format with every documented series present even at value 0 on a fresh registry (regex-validated per data line, not just "contains substring"); `observeHttpRequest` increments the error counter ONLY for 5xx; `observeMutationAck` records into the same histogram under both transport labels; `observeAiRun` reads token counts straight from `usage` and increments the cost counter proportionally (and records 0 cost for an empty-usage failed run); `job_queue_depth`'s live `collect()` reflects a real (faked) pg-boss `getQueueStats` sample once `setJobQueueDepthSource` is called, and never throws (leaves the gauge as-is) when a configured queue doesn't exist yet. `core/server.spec.ts` (+4) — `GET /metrics` returns valid Prometheus format with the right content-type; never session-protected (200, never 401); a real `GET /health/live` request increments its own `http_request_duration_seconds` series, observable on the next scrape; an injected `BuildServerOptions.metrics` is the exact same instance read back off `app.metrics`. New `core/metrics.int.spec.ts` (3 integration tests, real PGlite+Fastify+WS, mirrors `wsGateway.int.spec.ts`'s real-socket pattern) — a real REST `operations:batch` mutation increments `mutation_ack_duration_seconds{transport="rest"}` (plus the generic REST hook's own series for that route); a real WS `mutation` message (real ticket issuance → real WebSocket connect → real `mutation_ack` frame) increments the SAME histogram with `transport="ws"`; a real `createAiRun` call (mocked `fetchImpl`, same F2c pattern `pipeline.int.spec.ts` already uses) with deliberately non-round usage numbers (317/129/446 tokens) lands EXACTLY those numbers in `arch_canvas_ai_run_tokens_total{kind="prompt"|"completion"}` and the precisely-computed estimated cost (`0.00446`) in `arch_canvas_ai_run_estimated_cost_usd_total`, read back through a real `GET /metrics` call — proving the wiring reaches the scrape endpoint, not just that `observeAiRun` itself works (already covered at the unit level).

`pnpm -w lint`/`pnpm -w typecheck`/`pnpm -w build` all clean. AD-008 re-confirmed: `grep -rln excalidraw apps/server/dist/**/*.js` → the same 9 pre-existing allowlisted files, zero new hits. `pnpm -w test:unit` green (22/22 tasks, server 338/338 — up from 326 after T90, +12: 7 `metrics.spec.ts` + 4 `server.spec.ts` + 1 `no-egress.spec.ts`'s own per-new-file assertion count). `pnpm -w test:integration` green (14/14 tasks, server 317/317 across 42 files — up from 314 after T90, +3 new `metrics.int.spec.ts`). `@arch-canvas/backup` added as a real (non-dev) dependency of `@arch-canvas/server` was already done in T90; `prom-client@15.1.3` added here as a new real dependency.

**Deviation**: `@arch-canvas/backup`'s addition to `apps/server`'s dependency tree (T90) combined with `prom-client`'s own `pnpm add` momentarily surfaced a stale duplicate `drizzle-orm` peer-resolution instance in `.pnpm`'s store (two physical directories, one with the `@opentelemetry/api` optional peer resolved and one without), causing spurious `tsc --noEmit` type-identity errors in `ws-gateway`'s own pre-existing (unmodified) integration test file. A plain `pnpm install` (no lockfile edits) reconciled it — confirmed root-caused to store staleness, not a real code issue, by the fact that `pnpm-lock.yaml` itself only ever recorded ONE resolved `drizzle-orm@0.45.2(...)` variant throughout. No production code or lockfile change was needed beyond the `pnpm add prom-client` itself; documented here since it briefly looked like a real type error before being traced to its actual (transient, install-state) cause.
...
**Status**: ✅ Complete — researched the CURRENT `@opentelemetry` package layout before writing any code (Knowledge Verification Chain): `@opentelemetry/sdk-trace-base@2.10.0` is now a thin compat shim re-exporting `BasicTracerProvider`/`SimpleSpanProcessor`/`InMemorySpanExporter`/`ConsoleSpanExporter` from a new `@opentelemetry/sdk-trace` core package (a real v1→v2 restructuring, not assumed), and `BasicTracerProvider`'s v2 constructor takes `{ spanProcessors: SpanProcessor[] }` directly (no `addSpanProcessor()` method anymore). **Manual spans chosen over `@opentelemetry/sdk-node` auto-instrumentation** (the task's own explicitly-permitted alternative) — documented in `tracing.ts`'s own doc comment for two concrete reasons: auto-instrumenting `pg` captures SQL text/params as span attributes by default (replicating exactly the leak class `REDACT_PATHS` exists to prevent, at a different layer), and auto-instrumenting `http` would double-count REST latency already owned by T91's `MetricsRegistry`. New `apps/server/src/core/tracing.ts`: `createTracing(overrides)` (defaults to `InMemorySpanExporter` — spans are always created, never sent over a real network unless a caller explicitly injects a real exporter — satisfying "tracing desabilitado ... sem overhead de exporter real" by construction, no separate boolean toggle needed); `startChildSpan`/`withSpan`/`withOptionalSpan` — one reusable start/attribute/error/end primitive every boundary below uses, chaining spans into the same trace via EXPLICIT parent-`Span` passing (`Tracer#startSpan`'s own 3rd `context` argument, via `trace.setSpan(ROOT_CONTEXT, parentSpan)`) rather than registering a process-global `AsyncLocalStorage` `ContextManager` — deliberately avoids that extra dependency/cross-request-leakage-risk surface, matching this codebase's existing preference for explicit dependency threading (`deps.jobs`/`deps.metrics`) over ambient global state. `withOptionalSpan` is the degrade-friendly seam (`tracer: Tracer | undefined` — when absent, `fn` just runs directly, zero overhead) every module's own `tracing?: Tracing` dep uses, identical shape to every other optional observability seam in this codebase.

Wiring: `core/server.ts` decorates `app.tracing` (injectable via `BuildServerOptions.tracing`, same pattern as T91's `app.metrics`) and adds `onRequest`/`onResponse` hooks emitting one `http.request` span per REST request (method/route/status_code attributes only — never a header/body/query value), decorated onto `request.otelSpan` so a route handler can parent a child span under it. `registerModules.ts` reads `app.tracing` back (never constructs a second provider — would break trace chaining across modules) and threads it into `registerDiagramSyncModule`/`registerWsGatewayModule` (`db.append_operation` span — REST's is a child of its own `http.request` span via `request.otelSpan`; WS's is standalone, documented why: the WS upgrade's own `http.request` span already ended by the time any `mutation` message arrives), `registerExportModule` (`export.generate` span — the storage boundary, wraps `generateExports` + every `storage.putObject` call), and `registerAiEngineModule`→`pipeline.ts`'s `createAiRun` (the AI-provider-call boundary: a parent `ai.run` span with 3 chained children — `ai.build_context`/`ai.call_provider`/`ai.apply_patch` — restructured from 3 flat function calls into 3 `withOptionalSpan`-wrapped blocks, each returning exactly what the surrounding code already needed so no control flow changed). `registerGracefulShutdown` now also calls `app.tracing.shutdown()` on drain.

**Redaction discipline (the task's own explicit ask)**: every span attribute anywhere in this wave is a hand-picked id/count/outcome (`diagram.id`, `workspace.id`, `http.method`/`route`/`status_code`, `operation.delta_count`, `ai.model`, `ai.tool_call_count`, `export.id`) — never `params.userRequest`, scene/element content, or the provider token/encryption key. `ai-engine/tracing.int.spec.ts`'s first test seeds a REAL scene element carrying a distinctive marker label AND a distinctive prompt string, runs a real `createAiRun`, and greps the ENTIRE serialized span array (every name + every attribute value) for both markers plus the provider token plus the encryption key — all four absent, proving the redaction structurally, not by inspection.

Tests: `core/tracing.spec.ts` (8 unit) — `withSpan` emits one span with the right name/attributes/OK status; records exceptions + ERROR status on throw, still rethrows; a child span (explicit `parentSpan`) shares its parent's `traceId` and `parentSpanContext`; two independent spans get different `traceId`s (chaining is opt-in); `withOptionalSpan` runs `fn` with `span: undefined` and zero span emission when no tracer is given. `core/server.spec.ts` (+6) — a real REST request emits `http.request` with the right method/route/status_code; never sets ERROR on a 2xx; a real 5xx sets ERROR status; an injected `BuildServerOptions.tracing` is the exact instance read back off `app.tracing`; with no override, the server boots/responds normally (default exporter confirmed to be `InMemorySpanExporter`, never a real network client). `core/tracing.int.spec.ts` (2 integration, real PGlite+Fastify+WS) — a real REST `operations:batch` mutation emits `db.append_operation` as a genuine CHILD of that request's own `http.request` span (same traceId, matching parentSpanId); a real WS `mutation` message emits a standalone `db.append_operation` (no parent, documented why). New `ai-engine/tracing.int.spec.ts` (3 integration) — the full chained-trace + redaction proof described above; a FAILED AI run (provider HTTP error) still emits `ai.run`+`ai.call_provider` (never `ai.apply_patch`, proving spans mirror actual control flow) and still leaks nothing; `deps.tracing` omitted entirely → the run completes identically, proving the degrade is real.

`pnpm -w lint`/`pnpm -w typecheck`/`pnpm -w build` all clean. AD-008 re-confirmed: `grep -rln excalidraw apps/server/dist/**/*.js` → the same 9 pre-existing allowlisted files, zero new hits. `pnpm -w test:unit` green (22/22 tasks, server 352/352 — up from 338 after T91, +14: 8 `tracing.spec.ts` + 6 `server.spec.ts`). `pnpm -w test:integration` green (14/14 tasks, server 322/322 across 44 files — up from 317 after T91, +5: 2 `core/tracing.int.spec.ts` + 3 `ai-engine/tracing.int.spec.ts`). New dependencies of `@arch-canvas/server` (all real, non-dev — spans are a production concern): `@opentelemetry/api@1.9.1` (already resolved in the lockfile as `drizzle-orm`'s own optional peer, pinned explicitly here since this package now imports it directly), `@opentelemetry/sdk-trace-base@2.10.0`, `@opentelemetry/resources@2.10.0`.

**Deviation (disclosed, mirrors AD-007's Postgres/MinIO precedent)**: a real OTLP exporter talking to an actual OTel Collector (e.g. `@opentelemetry/exporter-trace-otlp-http`) is NOT wired in this wave — no real OTel Collector is reachable in this sandbox, so wiring one would be fabricating configuration against nothing to actually talk to. `Tracing.exporter` is fully injectable (production default `InMemorySpanExporter`), so adding a real exporter later is a constructor-argument change at the `index.ts` call site, not a redesign; documented explicitly in `tracing.ts`'s own doc comment rather than silently implied as complete.
...
**Status**: ✅ Complete — read `docs/product-spec.md` §11 directly for the 7 literal documented thresholds ("Alertas: ACK p95 > 2 s, erro save > 1%, fila de jobs atrasada, snapshot falhando, storage indisponível, backup/restore inválido e aumento de auth failures") before writing anything. New `infra/observability/alerts.yml` — a real Prometheus Alerting Rules file (`groups: [{ name, rules: [{ alert, expr, for, labels, annotations }] }]`, researched from Prometheus's own documented rule-file schema, Knowledge Verification Chain), 7 rules, one per threshold: `MutationAckLatencyP95High` (`histogram_quantile(0.95, ...) > 2`), `DiagramSaveErrorRateHigh` (error-rate fraction `> 0.01`), `JobQueueBacklogged`, `SnapshotCompactionFailing`, `StorageDependencyDown`, `BackupRestoreTestFailing`, `AuthFailureSpike`.

**Genuine gap found and closed, not papered over**: 3 of the 7 thresholds (failing snapshot, storage unavailable, invalid backup/restore) had NO corresponding `/metrics` series before this task — `core/metrics.ts` (T91) only ever tracked latency/duration/token histograms, never a dedicated failure signal for these 3. Rather than write alert rules against metrics that don't exist (which T93's own cross-check test would catch and fail), this task EXTENDS `MetricsRegistry` with 4 new series and wires each into its real source: `arch_canvas_snapshot_compaction_failures_total` (Counter, incremented in `snapshot/compaction.ts`'s `compactDiagram` catch block, rethrown unchanged); `arch_canvas_restore_test_failures_total` (Counter, incremented in `backup/restoreTest.ts`'s `recordOutcome` — T90's own doc comment had promised "o formato do evento é o que a T93 consome"; this is that promise made real as an actual `/metrics` series, not just an audit-event convention); `arch_canvas_auth_failures_total` (Counter, incremented in `auth/routes.ts` on BOTH the local-login and OIDC-login failure paths, alongside T85's existing audit event); `arch_canvas_dependency_up` (Gauge, labeled `name`, wired into `core/server.ts`'s EXISTING `GET /health/ready` handler — the SAME `DependencyCheck`/`dependencyChecks` results that route already computes now also set the gauge, so a `storage`-named check wired at the production call site, e.g. `index.ts`, makes `StorageDependencyDown` fire on real data with zero new health-check machinery). All 4 additions are optional-degrade (`metrics?:` params), matching every other observability seam in this codebase.

New `apps/server/src/core/alertRules.spec.ts` (5 unit tests, using the `yaml` package — added as a new devDependency of `@arch-canvas/server`, since parsing only happens in this test, never in production code — plus a hand-written Zod schema modeling the REAL Prometheus Alerting Rule shape, `for`/`interval` validated against Prometheus's own documented duration-literal grammar `\d+(ms|s|m|h|d|w|y)$`): (1) the file parses and validates against the schema; (2) a deliberately-malformed rules object (missing `expr`, an invalid `for` duration) is REJECTED — proves the schema check is real, not a rubber stamp; (3) exactly 7 rules exist, named to match the 7 documented thresholds, with the 2 numerically-specified thresholds' literal values (`0.95`/`2`, `0.01`) asserted present in their `expr` text; (4) **the cross-check**: every `arch_canvas_*` token referenced in every rule's `expr` (extracted by regex, base name recovered by stripping Histogram/Counter's `_bucket`/`_sum`/`_count` suffixes) is confirmed present in a FRESH `MetricsRegistry`'s own `# HELP` lines — a renamed/removed metric breaks this test, not silently producing a dead alert; (5) every rule carries `severity`/`summary`/`description`/`for` — never a silent or uninterpretable alert.

**Deviation (documented, disclosed)**: the source doc gives explicit numbers for only 2 of the 7 thresholds (ACK p95 > 2s, save error > 1%); the other 5 use qualitative language ("atrasada"/"falhando"/"indisponível"/"inválido"/"aumento") with no number. `JobQueueBacklogged` (>100 sustained 10m) and `AuthFailureSpike` (>5/s sustained 5m) use documented, reasonable interpretations, disclosed inline as YAML comments in `alerts.yml` itself — not invented ACs, and not silently presented as if the source doc specified them.

**Real-Prometheus-server disclosure** (same honesty-over-theater discipline as every other F5 task): this file is real, versioned, schema-valid, cross-checked configuration — a real Prometheus server actually scraping `/metrics`, evaluating these rules, and firing a real Alertmanager notification is production infrastructure this sandbox has no real Prometheus/Alertmanager to exercise against (same AD-007-style disclosed gap as T92's OTLP export); documented in `alerts.yml`'s own header comment, deferred to T96's compose `observability` profile.

`pnpm -w lint`/`pnpm -w typecheck`/`pnpm -w build` all clean (full workspace, reproduced fresh — not incrementally cached — for this task's own `Gate: build`). AD-008 re-confirmed: `grep -rln excalidraw apps/server/dist/**/*.js` → the same 9 pre-existing allowlisted files, zero new hits. `pnpm -w test:unit` green (22/22 tasks, server 357/357 — up from 352 after T92, +5 new `alertRules.spec.ts`). `pnpm -w test:integration` green (14/14 tasks, server 322/322 across 44 files — unchanged from T92's own count, this task added zero integration tests, unit only per its own declared `Tests: unit`; the metric-wiring changes to `compaction.ts`/`restoreTest.ts`/`auth/routes.ts`/`server.ts` are additive optional params, confirmed to not regress any existing integration test). Reproduced this batch's full gate discipline twice end to end (the first `pnpm -w test:integration` run hit a real host-level OOM kill — exit 137, `free -h` showed <1.5Gi available mid-run from unrelated concurrent sandbox memory pressure, NOT a code defect; re-ran clean once memory recovered, 14/14 tasks green both times for every task that completed). Cluster/process hygiene confirmed clean after every run: `pg_lsclusters` shows only `main` online (pre-existing), `backuptest`/`restoretest` down; zero `redis-server` processes.
...
**Status**: ✅ Complete — read `packages/test-fixtures/src/generateScene.ts` (F0, deterministic mulberry32-seeded generator) and `docs/product-spec.md` §11 directly before writing anything, quoting the exact SLO sentence verbatim in the new file's own header comment: "p95 ACK < 1 s na LAN com lote normal; bootstrap p95 < 3 s para diagrama de 5 mil elementos". New `apps/server/src/modules/diagram-sync/bootstrap.perf.int.spec.ts`, same PGlite+Fastify integration harness as `bootstrap.int.spec.ts`/`operations-batch.int.spec.ts` (AD-007). Scenes of 1k/5k/10k elements (`generateScene(elementCount, seed)`) are persisted by chunking into `MAX_OPERATION_ELEMENTS` (500, `@arch-canvas/diagram-domain`)-sized operation rows through the SAME `appendOperation` the real REST route calls — mirroring how a scene that large would actually have accumulated over many real batches, not one artificially oversized row; only the HTTP layer itself is skipped for setup speed, since seeding time isn't part of what's measured. For each scene size: 7 real `GET /diagrams/:id/bootstrap` samples (asserting the full persisted scene comes back) and 7 real `POST /diagrams/:id/operations:batch` samples with a typical 30-delta batch of brand-new elements layered on top of the large existing scene (the "user keeps editing a big diagram" case) — p95 computed from each sorted sample set and logged via `console.log` (not just pass/fail) in the format `[PERF-01 T94] elements=<n> bootstrap p95=<ms> samples=[...] ack p95=<ms> samples=[...]`. Bootstrap p95 asserted `< 3000ms` at both 1k and 5k (1k as a strictly-easier sanity check under the same documented 5k ceiling); 10k has no documented target in the source doc so it is measured/logged only, never asserted against an invented threshold. ACK p95 asserted `< 1000ms` at all three scene sizes (the §11 target is scene-size-agnostic — "lote normal" on top of whatever the diagram already holds). Measured results (this sandbox, single run): 1k → bootstrap p95 ≈26ms, ACK p95 ≈41ms; 5k → bootstrap p95 ≈101–122ms, ACK p95 ≈88–94ms; 10k → bootstrap p95 ≈184–201ms, ACK p95 ≈165–204ms — all comfortably inside their asserted ceilings (orders of magnitude of headroom, expected given PGlite's single-process/no-network/no-disk-fsync profile disclosed in the file's own header). **Disclosure** (file header, same honesty-over-theater discipline as T90/T91/T93): explicitly states this is a PGlite/single-sandbox-process PROXY — real Postgres semantics and code path, but no real network hop, disk fsync, connection-pool contention, or concurrent tenancy — and is evidence the code path isn't pathologically slow under a faithful-but-idealized proxy, not a production SLO guarantee or a substitute for genuine load testing.
`pnpm --filter @arch-canvas/server exec tsc --noEmit` clean; `pnpm exec biome check` clean. `pnpm --filter @arch-canvas/server exec vitest run -c vitest.integration.config.ts src/modules/diagram-sync/bootstrap.perf.int.spec.ts` → 3/3 green standalone. Full `pnpm -w test:unit` green (22/22 tasks, server 357/357 — unchanged, this task added zero unit tests, integration only per its own declared `Tests: integration`). Full `pnpm -w test:integration` green (14/14 tasks, server 325/325 across 45 files — up from 322/44 after T93, +3 new). AD-008 unaffected (test-only file, no server dist changes touch `SceneElement`/Excalidraw by value). **Deviation**: none functional — `SAMPLE_COUNT = 7` (not a number the task text specifies) chosen as a pragmatic balance between a meaningful p95 (`ceil(0.95 × 7) = 7`th-of-7, i.e. the max sample) and this proxy benchmark's own runtime, documented inline in the file.
...
**Status**: ✅ Complete — researched the CURRENT compatibility of the 3 named candidates against the installed stack (Vitest 3.2.4/jsdom 30/React 19, `apps/web/package.json`) before picking one (Knowledge Verification Chain, documented inline in the new spec file's own header): `vitest-axe` has never left pre-release (`1.0.0-pre.5`, last published Jan 2025, pinned to the older `axe-core@^4.10.2`); `@axe-core/react` is a live-page console-logging tool, not a unit-test assertion library, so it structurally can't fail a `vitest run`; `jest-axe` (published within the last month, tracking current `axe-core@4.12.1`) is a plain `expect.extend(toHaveNoViolations)` custom matcher with zero Jest-runtime coupling — confirmed directly from its own README that Vitest's Jest-API-compatible `expect` supports it natively, which IS the "small compat shim" the task text anticipated (no actual shim code needed beyond the one `expect.extend` call). Added `jest-axe@11.0.0`, `@types/jest-axe@3.5.9`, `@testing-library/react@16.3.2` (the first line to declare a React 19 peer range), `@testing-library/dom@10.4.1` as new `apps/web` devDependencies.

New `apps/web/src/a11y/shell.a11y.spec.tsx` — matches the project's existing `src/**/*.spec.tsx` Vitest include glob (no config change needed), scans all 3 named components: `AppShell`, `LanguageSwitcher`, and `DiagramEditorPage` (2 real render paths — route-params-present "loading" state with `fetch` deliberately stubbed to a permanently-pending promise so `<EditorSurface/>`/Excalidraw is never reached, and the "missing diagram id" no-route-match state — both genuine code paths the component already has, not fabricated for the test). A `seriousOrCriticalViolations()` helper filters axe-core's raw `results.violations` to `impact === 'serious' | 'critical'` before asserting `toEqual([])` — matching the AC's actual wording ("zero violações sérias/críticas"), narrower than jest-axe's own `toHaveNoViolations` matcher (which fails on ANY impact level). All 3 real components/4 render scenarios: zero violations at any impact level, not just serious/critical. **Proven-real gate** (mirrors `alertRules.spec.ts`'s T93 pattern of feeding the validator a deliberately-malformed input): a `ComponentWithMissingAltText` fixture (`<img>` with no `alt`, `biome-ignore lint/a11y/useAltText`d as deliberate) is scanned by the same helper — asserts axe-core reports a `serious`/`critical` violation with `id === 'image-alt'`, AND separately asserts jest-axe's own `toHaveNoViolations` matcher actually throws for it — proving the gate fails on a real violation at both the custom-filter layer and the underlying matcher layer, not a rubber stamp.

**Coverage disclosure** (file header comment, explicit, never implying total coverage): `apps/web/src` was confirmed by a full directory listing (before writing anything) to contain exactly 4 UI files today — `App.tsx` (routing only) plus the 3 scanned components. The full editor canvas (Excalidraw's own rendered drawing surface — deliberately never reached here since `fetch` stays pending), the AI generation dock, presentation/presenter mode, and the comments UI all have NO React component built yet anywhere in this codebase (confirmed by the same directory listing) — explicitly listed as NOT covered, with a note that each needs its own `*.a11y.spec.tsx` once real UI exists.

`pnpm --filter @arch-canvas/web exec tsc --noEmit` clean (one real finding along the way, documented inline in the file: `@types/jest-axe@3.5.9`'s own published types pin an internal, stale `axe-core@^3.5.5` purely for type resolution, unrelated to the real `axe-core@4.12.1` jest-axe runs — importing `AxeResults` from a separately-added `axe-core` devDependency produced a structural type mismatch between the two; fixed by deriving the result type as `Awaited<ReturnType<typeof axe>>` instead, which sidesteps the stale-types-package version entirely, and removing the now-unneeded explicit `axe-core` devDependency). `pnpm exec biome check` clean (one real fix along the way: biome's own `lint/a11y/useAltText` rule correctly flags the deliberately-broken fixture too — suppressed with a same-line `biome-ignore` comment directly on the flagged JSX node, since a 2-line wrapped reason comment was silently ignored by biome's suppression parser, confirmed empirically). `pnpm --filter @arch-canvas/web exec vitest run src/a11y/shell.a11y.spec.tsx` → 6/6 green standalone. Full `pnpm -w test:unit` green (22/22 tasks; web now 28/28, up from 22/22 — +6 new; server unaffected at 357/357), confirming these tests run as part of normal CI with no separate step, per this task's own explicit "Done when" line. `pnpm -w test:integration` unaffected (this task added zero integration tests, unit only per its own declared `Tests: unit`, apps/web has no integration test tier). **Deviation**: none functional — `axe-core` was briefly added as an explicit `apps/web` devDependency purely to chase the type-import approach described above, then removed once the `Awaited<ReturnType<typeof axe>>` derivation proved both simpler and correct; the package.json diff for this task reflects only the 4 packages actually needed.
...
**Status**: ✅ Complete — read `registerModules.ts`/`index.ts`/`core/server.ts` in full before assuming anything was missing (confirmed by direct read, not by trusting the task text): SEC-01/02 (headers/CORS/rate-limit), OIDC (T87/T88), `/metrics` (T91), tracing (T92) were ALL already correctly wired — only T90's `backup-restore-test` job registration was genuinely missing, exactly as T90's own Status note predicted ("its own registration is T96's job").

**Restore-test job wiring**: `core/config.ts` gains 3 new optional env vars — `RESTORE_TEST_TARGET_DATABASE_URL`, `RESTORE_TEST_BACKUP_DIR`, `RESTORE_TEST_CRON` — surfaced as `AppConfig.restoreTest` only when the first two are BOTH set (same "trio/pair together means configured" convention as `config.oidc`). New `getLatestBackupPathFromDirReal` (`modules/backup/restoreTest.ts`) is a real, filesystem-based `getLatestBackupPath` resolver — real `readdir`/`stat`, picks the most recently modified `*.zip` in a directory, throws a clear error on an empty/missing directory — one reasonable, disclosed interpretation of the module's own "no backup catalog" scope note (3 new unit tests, real temp directories via `mkdtemp`, no mocking). `registerModules.ts` now imports `registerRestoreTestJob`/`RESTORE_TEST_JOB` and, inside the existing `if (deps.jobs)` block, additionally gates on `config.restoreTest` before calling `registerRestoreTestJob` — building a real `BackupObjectStore` via `@arch-canvas/backup`'s own `createS3Client`/`createS3ObjectStore` against `config.s3` (aliased `createBackupS3Client`/`createBackupS3ObjectStore` to avoid a real naming collision with `storage/client.ts`'s own differently-shaped `createS3Client`, caught by `tsc`). `RESTORE_TEST_JOB` added to `metrics.setJobQueueDepthSource`'s queue list unconditionally (harmless — degrades gracefully per `MetricsRegistry`'s own documented per-queue collect() behavior when the queue was never `defineJob`'d). New `core/registerModules.restoreTest.int.spec.ts` (3 integration tests, real PGlite-backed pg-boss via `fromPglite`) proves the wiring gate itself — never fires the job (that's `restoreTest.int.spec.ts`'s job against 2 real Postgres 16 clusters, T90) — just that `boss.getSchedules(RESTORE_TEST_JOB)` returns exactly 1 entry with the configured cron when BOTH `config.restoreTest` and `deps.jobs` are present, 0 entries when either is missing, and that boot completes with a fully working server (`GET /health/live` → 200) in the "config set, no jobs" case.

**Compose profiles (docs/product-spec.md §12)**: added `observability` (`prometheus:v3.7.3`, `grafana:12.3.0`, `otel-collector-contrib:0.119.0`) and `oidc-dev` (`keycloak:26.0`) to `infra/compose/compose.yaml`, each service tagged `profiles: [...]` so neither starts under a bare `docker compose up`. New config files: `infra/observability/prometheus.yml` (scrapes `server:3000/metrics`, loads T93's `alerts.yml` as its rule file), `infra/observability/otel-collector-config.yml` (real OTLP gRPC+HTTP receiver → `debug` exporter pipeline), `infra/observability/grafana-datasources.yml` (provisions Prometheus as Grafana's default datasource) — all bind-mounted read-only from the new `prometheus`/`grafana`/`otel-collector` services. `keycloak` sets BOTH the legacy (`KEYCLOAK_ADMIN`/`KEYCLOAK_ADMIN_PASSWORD`) and current (`KC_BOOTSTRAP_ADMIN_USERNAME`/`KC_BOOTSTRAP_ADMIN_PASSWORD`) bootstrap-admin env var names, disclosed inline as a deliberate hedge since this sandbox cannot pull the image to confirm which one the resolved tag expects. `infra/compose/.env.example` and `README.md` updated with the new optional variables/profiles table. New named volumes `prometheus-data`/`grafana-data`.

**Compose validation (disclosed limitation, same discipline as `infra/compose/README.md`'s own pre-existing "What's verified here vs. what isn't" section — this sandbox has NO Docker daemon)**: `docker compose -f compose.yaml config` (base, no profile — confirms the 4 new services are correctly ABSENT), `--profile observability config`, `--profile oidc-dev config`, and both profiles together `--profile observability --profile oidc-dev config` were all run for real in this sandbox (the `docker compose` CLI plugin itself needs no daemon to parse/interpolate/resolve — only `up`/`build`/`pull` do) — all 4 runs exited 0 with zero errors/warnings, every bind-mounted config path resolved to a real file that exists on disk (`/home/user/ia-draw/infra/observability/{prometheus,otel-collector-config,grafana-datasources}.yml`), and `.env.example`'s new vars interpolated correctly. What was NOT verified, because it requires the daemon: any image actually pulled, any container booted, Prometheus actually scraping `/metrics`, Grafana actually rendering the datasource, the OTel Collector actually receiving a span, or Keycloak actually serving a login page — stated explicitly in both `compose.yaml`'s own comments and `README.md`, never implied as tested.

**Real-boot smoke test** (same procedure T81's/T70's Status notes established for their own closing tasks): `pnpm -w build` (12/12 tasks), AD-008 re-confirmed (`find apps/server/dist -name "*.js" | xargs grep -l excalidraw` → the same 9 pre-existing allowlisted comment-only files, zero new hits). Ran the real migration against this sandbox's pre-existing `main` Postgres 16 cluster (`arch_canvas`/`arch_canvas` role, already provisioned from a prior wave's smoke test), then booted `node apps/server/dist/index.js` for real (`NODE_ENV=development`, real `DATABASE_URL`/`S3_*`/`SESSION_SECRET`/`ENCRYPTION_KEY`/`PUBLIC_URL` env vars, `PORT=3900`) — genuinely listening, not injected. Drove a full real HTTP flow: seeded a local user via the compiled `createLocalAccount` against the real DB, `POST /auth/login` (real session cookie), `POST /workspaces`/`/projects`/`/diagrams`, `GET .../bootstrap`, and a real `POST .../operations:batch` mutation — all 200s with real response bodies, and a deliberate wrong-password `POST /auth/login` (401, to exercise the auth-failure metric for real). Then the 3 required checks, all against the real running process:
- `curl -I http://localhost:3900/health/live` → CSP, `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`, HSTS-adjacent headers all present (T82's headers, confirmed on a real response, not a test double).
- `GET /metrics` → valid Prometheus text-exposition format (`content-type: text/plain; version=0.0.4; charset=utf-8`; a small Node script parsed all 189 non-comment data lines against the real Prometheus line grammar — 0 malformed), with NON-ZERO series after the activity above: `arch_canvas_http_request_duration_seconds_count{...route="/diagrams/:id/operations:batch"...} 1`, `arch_canvas_mutation_ack_duration_seconds_count{transport="rest"} 1`, `arch_canvas_auth_failures_total 1` — not just present-at-zero, genuinely incremented by real activity.
- `GET /auth/oidc/login` (no `OIDC_ISSUER_URL`/etc. set) → `503 Service Unavailable` with security headers still attached, never a crash, never a bare 404.

Server shut down cleanly afterward (`pkill -f "apps/server/dist/index.js"`); confirmed via `ps aux` that no `node .../dist/index.js` process remained.

**`spec.md` Requirement Traceability**: all 15 F5 rows (SEC-01..05, OIDC-01..03, DR-01..02, OBS-01..03, PERF-01, A11Y-01) flipped `Pending` → `Implementing`, each annotated with its authoring task/commit — deliberately NOT `Verified` (reserved for the independent Verifier pass, per this batch's own explicit instruction and every prior wave's precedent). The **Coverage** summary line updated to state F0–F5 are now ALL at Implementing-or-better (92/92 requirements), closing the roadmap's implementation while explicitly flagging the outstanding F5 Verifier pass as the only remaining step.

**Full `Gate: build` reproduced clean, end to end**: `pnpm -w lint` clean (406 files, biome). `pnpm -w typecheck` clean (23/23 tasks). `pnpm -w build` clean (12/12 tasks); AD-008 re-confirmed. `pnpm -w test:unit` green (22/22 tasks — server 360/360, up from 357 after T95, +3 new `getLatestBackupPathFromDirReal` tests; web 28/28 unchanged from T95). `pnpm -w test:integration` green (14/14 tasks — server 328/328 across 46 files, up from 325/45 after T94, +3 new `registerModules.restoreTest.int.spec.ts`; database 36/36 across 7 files and backup 8/8 across 2 files both unaffected, confirming T96's changes never touched those packages' own behavior). Zero lingering `vitest`/`turbo`/`node .../dist/index.js` processes after every run (`ps aux` re-checked); `pg_lsclusters` shows only `main` online, `backuptest`/`restoretest` down (pre-existing from T89/T90, untouched by this task); zero `redis-server` processes.

**Deviation 1** (documented above, structural): the literal `if (deps.jobs) await registerRestoreTestJob(...)` shape T90's Status note quoted is necessarily widened to `if (deps.jobs) { ... if (config.restoreTest) { await registerRestoreTestJob(...) } ... }` — `registerRestoreTestJob` requires an isolated target database URL and a backup-location resolver that this codebase cannot safely default (a missing/misconfigured target must never silently fall back to production, or silently resolve to nothing); this is the narrowest change that keeps the exact quoted call intact while making the extra required config an explicit, disclosed precondition, mirroring `config.oidc`'s own established pattern in this same file.

**Deviation 2** (disclosed, unavoidable given the sandbox): the `oidc-dev` Keycloak service sets both the legacy and current bootstrap-admin-credential env var names side by side, since the exact tag's expected variable could not be confirmed without pulling the image — documented inline in `compose.yaml` and in this note, not silently guessed at as certain.
