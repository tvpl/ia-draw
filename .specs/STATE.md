...
- **Completed (todas com Verifier independente + sensor de mutação, PASS)**:
  ...
  - **F2c batch 1 (T49-T52: cliente do provider, contexto, tools de leitura/escrita)** — completo, commitado, pushed, gate verde (252 testes).
- **In-progress**: F2c batch 2 (T53-T57: pipeline de aplicação, aprovação/undo, suíte de prompt injection, evals determinísticos) rodando em sub-agent background — **fecha toda a onda F2 ao concluir**.
- **Next step**: ao completar F2c batch 2 → gate, push, Verifier da onda F2c completa (fecha F2 = todo o diferencial de IA do produto) → autorar/disparar F3 (docs/apresentação/protótipos), F4 (realtime), F5 (hardening) na mesma cadência: autoria → validate_tasks → batch(es) → gate real → Verifier independente com sensor de mutação → fix-loop se FAIL (máx. 3 iterações) → próxima onda, sem pausar salvo bloqueio genuíno.
- **Lições registradas** (`.specs/lessons.json`, todas `candidate`): L-001..L-014, incluindo L-008 (wiring de produção, aplicada proativamente desde F1c) e as mais recentes sobre asserção de shape de resposta vs. substring (F2a) e cobertura de propriedade incompleta (F2b/AIG-03).
- **Blockers**: none
