...
- **Phase / Task**: **Onda F6 fechada e verificada** (ver marco abaixo). **Onda F7 fechada e verificada.** **Onda F8 (GOV-01..06, API-01..03): implementação completa** — T1-T28 (Phases 1-4: gerador OpenAPI, `routeSchemas` por módulo, portão de CI de paridade OpenAPI, limiar de sucesso dos evals de IA) mais esta Phase 5 (T29-T35, Governança) — **todas as 35 tasks de `tasks.md` marcadas `[x]`, um commit atômico por task**. **O Verifier independente de F8 ainda não rodou** — é o próximo passo, não reivindicar F8 como verificado/fechado até ele reportar PASS.
- **Entregue em F7**:
...
- **Entregue em F8 (implementação, Verifier pendente)**:
...
- **Proximo passo**: dispatch do Verifier independente de F8 (author != verifier) — coverage check ancorado no spec (GOV-01..06, API-01..03) + sensor de discriminação, escreve `validation.md`, e só então `validate_state.py platform-maturity` fecha o ciclo desta onda. Depois de F8 verificado: **F9** (MCP-01..08) ainda **nao tem `tasks.md`** — depende de F6 (feito) e precisa de fase de Design propria antes de Tasks (cria `apps/mcp`, aplicacao nova, stdio).
