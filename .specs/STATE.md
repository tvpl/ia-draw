...
- **Proximo passo**: F6-F9 e agora **R1 (`ai-dock`) e R2 (`sso-sign-in`) de F10** estão todas fechadas e verificadas. Próximo é **R3 (`workspace-navigation`)** — spec ainda não escrita; pesquisa de backend já feita nesta sessão (hierarquia workspace→projeto→diagrama estrita, soft-delete via `deletedAt` sem rota de restore, `POST /workspaces` é a única forma de um usuário novo sair de "zero workspaces", roteamento aninhado é conceito novo pro frontend). Depende de R2 (feito). Ordem restante do roadmap depois de R3: R4 (`workspace-members`) → R5/R6/R7 (paralelas entre si) → R8/R9/R10/R11 → R12 → R13/R14 → R15/R16.
