...
- **Next step**: autorar F3 (docs/apresentação/protótipos/lint/AaC) → validate_tasks → batch(es) → gate real → Verifier independente com sensor de mutação → fix-loop se FAIL (máx. 3 iterações) → repetir para F4 (realtime) e F5 (hardening), sem pausar salvo bloqueio genuíno.
- **Lições registradas** (`.specs/lessons.json`, todas `candidate`): L-001..L-016, incluindo L-008 (wiring de produção, aplicada proativamente desde F1c) e as mais recentes sobre asserção de shape de resposta vs. substring (F2a), cobertura de propriedade incompleta (F2b/AIG-03), RunStore in-memory (aceitável para MVP single-process) e violação de author≠verifier corrigida (T57 se auto-marcou Verified antes do Verifier rodar — sem impacto funcional, corrigido).
- **Blockers**: none
