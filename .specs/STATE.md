...
- **Phase / Task**: **Onda F6 fechada e verificada** (ver marco abaixo). **Onda F7 fechada e verificada nesta sessao.** AGT-01..05 implementados inline (4 commits — sem `tasks.md` formal, wave pequena o bastante pra pular Tasks por regra de sizing) e verificados por Verifier independente: **PASS** — 5/5 ACs com evidencia `file:line`, gate verde (`lint`/`typecheck`/`test-unit`, Node 22 via fnm), sensor adaptado (wave so tem docs/config, sem camada de teste) achou 1 defeito real (`/audit` documentado sem `run`, ja corrigido em commit separado antes do fix virar parte do PASS). `validate_state.py platform-maturity` sai 0.
- **Entregue em F7**:
...
- **Proximo passo**: **F8** (GOV-01..06 + API-01..03) e **F9** (MCP-01..08) ainda **nao tem `tasks.md`**. F8 e Large scope de verdade — GOV e majoritariamente convencao (CODEOWNERS, template de PR, changesets, spec-por-dominio, formato de ADR, STATE.md por frente), mas API-01..03 exige gerar OpenAPI 3.1 a partir dos schemas Zod ja existentes em `apps/server` (nenhum `fastify-type-provider-zod` ou equivalente esta amarrado nas rotas hoje — e uma decisao de design real, nao so tasks). F9 depende de F6 (feito) e precisa de fase de Design propria antes de Tasks (cria `apps/mcp`, aplicacao nova, stdio). F8 nao toca as mesmas superficies que F7 (que ja fechou) nem que F9 — pode iniciar Design/Tasks a qualquer momento.
...
- **Armadilhas de ambiente**: Node **22.x** obrigatorio (`fnm use 22 && corepack enable`) — Node 24 quebra `apps/server/src/modules/ai-engine/callProvider.spec.ts` por `AbortSignal` cross-realm sob jsdom (confirmado de novo nesta sessao: o Node padrao do sandbox e 24.9.0). `make ci` **nao passa nesta maquina** por falta de `pg_lsclusters` e `redis-server` (erros `ENOENT` de spawn, anteriores a este trabalho); o gate substituto e `make lint && make typecheck && make test-unit`. Isso agora esta documentado no `CLAUDE.md` versionado, nao so aqui.
