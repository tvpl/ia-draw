...
- **Phase / Task**: **R17 (`editor-stability`) e R18 (`edge-routing`) fechadas e verificadas.** As 5 entradas restantes (R19 `instance-bootstrap`, R20 `green-gate`, R21 `ui-foundations`, R22 `rbac-clarity`, R23 `docs-truth`) estão com Specify/Design/Tasks prontos e Execute não iniciado.
  - **R17 — PASS**, 14 ACs com evidência `file:line`, sensor 7/7 mortas. A rota do editor monta; `crash-recovery.spec.ts` voltou ao verde sem uma asserção tocada; toda rota tem `RouteErrorBoundary` próprio; um e2e com allowlist de console vazia reprova em qualquer erro. Uma regressão foi introduzida por T2 e corrigida na mesma onda (`SharedResourcePage`, isolada por `git worktree` na baseline real conforme L-024).
  - **R18 — PASS**, 10 de 12 ACs com asserção direta e 2 marcadas estruturais por exigirem o stack de pé (EDGE-03/04, execução em R20/GATE-10). Sensor 3/3 mortas, uma delas só depois de expor um guard que aprovava a própria divergência que devia barrar. O `Caddyfile` deixou de rotear `/api` e passou a rotear os 15 prefixos reais; o proxy de dev deixou de manter lista própria.
- **Execução sem sub-agentes**: o oferecimento obrigatório de sub-agentes não pôde ser feito — a ferramenta não estava disponível nesta sessão. As 11 tasks rodaram inline e a validação foi o passe standalone de `validate.md`, com autor e verificador sendo o mesmo agente. **Isso é uma limitação registrada, não uma dispensa**: já custou um `Done when` marcado sem o artefato existir (changeset de T1, ver `editor-stability/validation.md` Gap 4).
...
- **Next step**: R19 (`instance-bootstrap`, 7 tasks) — desbloqueada agora que R18 fechou. Depois R20, R21, R22, R23 na ordem do roadmap.
...
- **Aberto e sem dono**: `SharedResourcePage` navega entre frames trocando `initialElements`, mas o `<Excalidraw/>` real lê `initialData` só no mount — o mock do teste relê a prop, então PRZ-33 passa no teste e não funciona no produto. Registrado no `remediation-roadmap.md` como candidato a R24. Não corrigido porque está fora do escopo de toda spec atual.
- **Lições novas**: L-046 a L-050 (`candidate`) — memoizar prop pelo valor e nunca pelo mount; mock mais permissivo que a lib esconde defeito; `Done when` que produz artefato tem de ser conferido contra o disco; guard por substring de identificador aprova redefinição local; AC que exige serviço de pé é estrutural, não verde.
- **Blockers**: nenhum.
