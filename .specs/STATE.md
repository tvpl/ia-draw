...
- **Phase / Task**: **Onda F6 fechada e verificada** (ver marco abaixo). **Onda F7 fechada e verificada.** **Onda F8 (GOV-01..06, API-01..03): implementação completa, Verifier rodou e reportou `⚠️ Issues`** — todas as 35 tasks de `tasks.md` marcadas `[x]`, e as 9 ACs (GOV-01..06, API-01..03) estão individualmente `✅ Verified` por evidência `file:line` (ver `validation.md`'s "F8 Wave Report"), **mas a onda NÃO está fechada**: o Verifier encontrou e confirmou (via `git worktree` na baseline real `a55c4d7`, não `git stash`) que este wave regrediu o piso de cobertura de `functions` travado por CIQ-04 (F6) de 39.73% para 28.87% — `pnpm --filter @arch-canvas/server run test:unit` sai 1 hoje. `tasks.md:453`'s nota (T26) que classificava isso como "pré-existente" foi refutada pelo Verifier: a isolação por `git stash` comparou contra um estado intermediário do próprio wave (38 commits dentro), não contra o início real da onda. **Não reivindicar F8 como fechada até o Fix Plan 1 de `validation.md` ser aplicado e o gate rodar limpo de novo.**
- **Entregue em F7**:
...
- **Entregue em F8 (implementação completa, Verifier reportou `⚠️ Issues` — 1 Fix Plan bloqueante pendente)**:
...
- **Proximo passo**: aplicar o Fix Plan 1 de `validation.md`'s "F8 Wave Report" (regressão do piso de cobertura `functions`, Blocker) — ou (a) adicionar testes cobrindo as ~124 funções recem-contadas pelo v8 apos os `routeSchemas` exports, ou (b) ajustar deliberadamente o ratchet em `apps/server/vitest.config.ts:26` com justificativa, corrigindo tambem a nota errada de `tasks.md:453`. Depois, re-rodar so o Gate Check (`pnpm --filter @arch-canvas/server run test:unit` saindo 0) — as 9 ACs, o sensor e o code quality ja verificados nao precisam ser refeitos. So entao F8 fecha de verdade. Depois de F8 fechada: **F9** (MCP-01..08) ainda **nao tem `tasks.md`** — depende de F6 (feito) e precisa de fase de Design propria antes de Tasks (cria `apps/mcp`, aplicacao nova, stdio).
...
- **Licoes**: L-017..L-023 registradas como `candidate` em `.specs/lessons.json`.
